...
class GridWithWeights(object):
    ...
    def cost(self, current, next_node, closed_set, visited_list):
        ...
        reachable, distance = closest_unvisited(visited, current, next_node)
        if not reachable:
            if current[4] == env.starting_pos: return None, False

            # distance = (1 - get_distance(env.starting_pos, current[4])/MAX_DISTANCE)
            distance = 0
            reachable = within_range(env.starting_pos, (None, None, current[2]+1, None, next_node))
...
def closest_unvisited(visited, current, next):
    ...
    if not bool(distances):
        ...
        min_distance =  1 - (distances[min(distances, key=distances.get)]/MAX_DISTANCE)
