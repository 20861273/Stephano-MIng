        ...
        with T.no_grad():
            ...
            if save_plot:
                for cntr, path in enumerate(paths):
                    if path[-1] == 0:
                        del path[-1]
                        PR.print_graph(False, policy, path, traj_actions[cntr], starting_positions[cntr], load_path, cntr, env)
                    else:
                        del path[-1]
                        PR.print_graph(True, policy, path, traj_actions[cntr], starting_positions[cntr], load_path, cntr, env)
