...
class Astar:
    ...
    def a_star(self, start, end, grid, direction=None, cr=0, step=0, occupied_cells=None):
        ...
        while self.heap:
            ...
            self.neighbors = self.graph.neighbors(cr, current, step, path_step, occupied_cells)
...
print("Testing time: %.2fh%.2fm%.2fs" %(th,tm,ts))
print("Average planning time: %.2fh%.2fm%.2fs" %(ph,pm,ps))
print("Average steps: %.2f" %(np.mean(np.array(steps_list))))
print("Average flight time: %.2fh%.2fm%.2fs" %(fh,fm,fs))
print("Average flight distance: %.2f m" %(np.mean(np.array(flight_distances))))
for ri in range(nr):
    print("Average explorations for drone %d: %.2f" %(ri, average_explorations[ri]))
print("Average time scheduling: %.8fs"%(np.mean(np.array(schedule_times))))
print("Average time path planning: %.8fs"%(np.mean(np.array(path_times))))